...
void* ThreadsConstructA(void* threadsstruct)
{
    ...
    for (auto it : p->sub_factors)
    {
        ...
        for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
        {
            ...
            // 然后构建b矩阵
            // ? : 为什么不是-J^Tr
            // 解释：由于这里并不需要求解状态量的增量，因此构建的GN方程为Aδx = b, 最终目标是为了得到marg之后的H_p和b_p, (下标p表示prior)
            // 再根据H_p=J^T J分解得到J_p, 根据b_p = J^T r 得到r_p
            // 由边缘化（舒尔补）之后的Hm和bm，反解得到的残差，再由于随着迭代的过程，剩余的状态量被不断更新。
            // 随着变量 x_r 的后续不断优化变化，残差r_p 或者 b_p 也将跟着变化，但雅克比 J_p 则固定不变了。
            // 因此边缘化因子MarginalizationFactor中，最终求得的残差residual = r_p + J_p * dx
            // dx为剩余状态量（marg之后剩下的状态量）的当前值x减去边缘化时的值x_0(线性化点x_0)
            // 即：dx为当前参数值与线性化点x0的差值
...
void MarginalizationInfo::marginalize()
{
    ...
    */
    ...
    // 以下部分是为了对A(即为先验Hessian矩阵H_p)进行奇异值分解，得到先验雅可比J_p.再结合先验b_p，即可得到先验残差r_p = J_p ^ (-1) * b_p
    // 有了先验残差linearized_residuals和先验雅可比linearized_jacobians，就可以在下次优化时构建边缘化因子了。
 ...
 * 
 * 先构建被marg状态量，以及与被marg状态量有关联的状态量的Hessian矩阵，然后进行边缘化，再将边缘化结果构建成边缘化因子，得到残差和雅可比
 * 线性化点x0处的雅可比固定下来了，但是残差r_p会随着变量x的优化不断被更新
 * 
 * @param[in] parameters 
 * @param[in] residuals 
 * @param[in] jacobians 
